...
Future<void> checkAndDownloadSDK(String location) async {
  ...
  if (!exists) {
    await downloadFile(
        Uri.parse('https://www.dropbox.com/s/rcauq6jfimxe099/MobileRTC?dl=1'),
        iosSDKFile);
  }
  ...
  if (!exists) {
    await downloadFile(
        Uri.parse('https://www.dropbox.com/s/zehuyw5uvj6dp6u/MobileRTC?dl=1'),
        iosSimulateSDKFile);
  }
  ...
  if (!exists) {
    await downloadFile(
        Uri.parse(
            // 'https://www.dropbox.com/s/mp50l6kdxmw9gv8/commonlib.aar?dl=1'),
            'https://www.dropbox.com/s/bjrlf0n0kdw9cyv/commonlib.aar?dl=1'),
  ...
  if (!exists) {
    await downloadFile(
        Uri.parse(
            'https://www.dropbox.com/s/v9hkrtne6oo968q/mobilertc.aar?dl=1'),
            // 'https://www.dropbox.com/s/crk6rn9p0ycf59y/mobilertc.aar?dl=1'),
        androidRTCLibFile);
  }
}
